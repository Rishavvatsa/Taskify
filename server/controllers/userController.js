...
const register = async (req, res) => {
  ...
  if (!validator.isStrongPassword(password)) {
    return res
      ...
      .json({
        error:
          "Enter a strong password that contains uppercase and lowercase letters, numbers and special characters",
      });
  }
  ...
  if (emailExists) {
    return res
      .status(400)
      .json({ error: "Email exists already, use another email" });
  }
...
const login = async (req, res) => {
  ...
  if (!user) {
    return res
      .status(404)
      .json({ error: "Email does not exist, try registering" });
  }
